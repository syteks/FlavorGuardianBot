...
export class MongoDBClient {
    ...
    public insert(collection: string, model: Meme, result: (error: any, data: any) => void): void {
        this.db.collection(collection).insertOne(model, (error, insert) => {
            return result(error, insert.ops[0]);
        });
    }

    public update(collection: string, objectId: string, model: Meme, result: (error: any, data: any) => void): void {
